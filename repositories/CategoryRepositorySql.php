...
class CategoryRepositorySql implements CategoryRepositoryInterface
{
    ...
    {
        return $this->db->createCommand('SELECT categories.*, count(posts.id) as posts_count FROM categories LEFT JOIN posts on posts.category_id = categories.id GROUP BY categories.id LIMIT :from, :limit', [':from' => $from, ':limit' => $limit])->queryAll();
    }
}